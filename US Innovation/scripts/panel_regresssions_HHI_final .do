clear all 

global path "/Users/marcocerundolo/Dropbox/DL_marco"

global output "$path/output/outreg/regressions/baseline"

use "$path/output/intermediate/panel/built5.dta", clear

* Merge distinct_firms data

merge m:1 county_fips using "$path/output/intermediate/hhi_aggregate_all1_pre80.dta"

keep if _merge == 3
drop _merge

egen state_yr = group(state fyear)

*** select sample

keep if semi_intens == 1

/*
bysort county_fips: gen patent = (num_patents>0 & fyear < 1976)
bysort county_fips: egen patent_yrs = sum(patent) 

gen intensive = (patent_yrs >= 1)

keep if intensive == 1
*/

***

sort comp_wtd 
gen group = ceil(4 * _n/_N)

* 1) Create group dummies
tabulate group, generate(g)    // creates g1…g4

* 2) Create the "treatment × group" and "instrument × group" terms
foreach i of numlist 1/4 {
    gen xg`i' = ltotal_dollars * g`i'
    gen zg`i' = log_spending_iv5 * g`i'
}

//ivreghdfe lw_cites_sub (xg1 xg2 xg3 xg4 = zg1 zg2 zg3 zg4), abs(fyear county_fips) cluster(county_fips) 

//ivreghdfe lw_cites_sub avg_wages  pop emp (xg1 xg2 xg3 xg4 = zg1 zg2 zg3 zg4), abs(fyear county_fips) cluster(county_fips) 

ivreghdfe lw_cites_sub avg_wages pop emp (xg1 xg2 xg3 xg4 = zg1 zg2 zg3 zg4), abs(fyear county_fips state_yr) cluster(county_fips) 	

estimates store iv1

estadd ysumm
local ymean=e(ymean)
local ysd=e(ysd)

outreg2 using "$output/hhi_final.tex", lab dec(5) nocons ///
		title(Intensive Margin Results) ctitle(Citation Weighted Patents)

/// Baseline

* Manually define coefficient and standard error
matrix b_extra = (0.1122621)
matrix se_extra = (0.0369543)

* Assign names so coefplot knows what it's referring to
matrix colnames b_extra = xg5
matrix colnames se_extra = xg5

coefplot iv1 ///
    (matrix(b_extra) , se(se_extra) label("Group 5")) ///
    , keep(xg1 xg2 xg3 xg4 xg5) ///
    vertical ///
    ci ///
    ciopts(recast(rcap) lwidth(medium)) ///
    xline(0) ///
    xlabel(1 "q1" 2 "q2" 3 "q3" 4 "q4" 5 "Baseline", angle(0)) ///
    xtitle("Group") ///
    ytitle("Estimated Treatment Effect") ///
    legend(off)
	
graph export "$path/output/figures/hhi/hhi.png", as(png) replace

*** Balance

estpost tabstat inventor_share tech_emp_share emp_share pop, by(group) statistics(mean) columns(statistics)

esttab using "$path/output/tables/group_means.tex", ///
    cells("mean") ///
    nomtitle ///
    nonumber ///
    noobs ///
    label ///
    title("Group Means") ///
    alignment(D{.}{.}{-1}) ///
    replace

esttab sum_stats using "$path/output/tables/summary_stats.tex", ///
  replace booktabs ///
  cells("mean(fmt(%9.2f)) sd(fmt(%9.2f)) min max") ///
  label nonum collabels("Mean" "Std. Dev." "Min" "Max") ///
  title("Summary Statistics") ///
  nonotes
  
 * Clear any old estimates
est clear

* List of variables
local vars inventor_share tech_emp_share emp_share pop

* Loop over groups
forvalues g = 1/4 {
    estpost summarize `vars' if group == `g'
    est store group`g'
} 

esttab group1 group2 group3 group4 using "$path/output/tables/summary_stats.tex", ///
    cells("mean") ///
    collabels("Group 1" "Group 2" "Group 3" "Group 4") ///
    nomtitles ///
    nonumber ///
    noobs ///
    label ///
    title("Group Means by Variable") ///
    alignment(D{.}{.}{-1}) ///
    replace
	
* 2) "Post" the means by group into Stata's e() matrix
estpost tabulate ///
    group, ///
    summarize inventor_share tech_emp_share emp_share pop ///
    statistics(mean)

* 3) Dump that matrix out as a LaTeX table
esttab using group_means.tex, ///
    cells("mean")                              /// only print the means
    collabels("Group 1" "Group 2" "Group 3" "Group 4") ///
    nomtitle    /// drop the "Model" column heading
    nonumber    /// drop column of row numbers
    noobs       /// suppress the "Obs = …" footer
    label       /// use variable labels (if you've set them)
    title("Group Means by Variable") ///
    replace	
